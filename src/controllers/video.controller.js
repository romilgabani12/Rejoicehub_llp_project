...
const publishAVideo = asyncHandler(async (req, res) => {
    ...
    const photoLocalpath = await req.files?.photo[0].path;

    if (!videoFileLocalPath && !photoLocalpath) {
...
const updateVideo = asyncHandler(async (req, res) => {
    ...
    if (!photo?.url) {
        throw new ApiError(400, "error while uploading a photo")
    }

    const videoIdDetails = await Video.findById(videoId);
    // console.log(videoIdDetails);

    const isDeleted = await deleteFromCloudinary(videoIdDetails?.photo);

    if (!isDeleted) {
        throw new ApiError(400, "Error while deleting the photo on cloudinary");
    }
...
const deleteVideo = asyncHandler(async (req, res) => {
    ...
    const isphotoDeleted = await deleteFromCloudinary(videoIdDetails?.photo, "image");
    // console.log(isphotoDeleted);

    const isVideoDeleted = await deleteFromCloudinary(videoIdDetails?.videoFile, "video");

    // console.log(isVideoDeleted);

    if (!isVideoDeleted || !isphotoDeleted) {
...
/*******************************************
 Toggle Publish status isprivate = true/false 
**********************************************/
    ...
    return res
        ...
        .json(
            new ApiResponse(
                200,
                video,
                "isPrivate status toggle successfully"
            )
        )
